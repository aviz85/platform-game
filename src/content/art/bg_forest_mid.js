...
export function build() {
  ...
  const crystalDown = (x, y, h, w0) => {
    ...
    for (let j = 0; j < h; j++) {
      ...
      // left face lit, right face dark, bright core line
  ...
  const island = (cx, topY, rw, depth, opts = {}) => {
    ...
    for (let i = -rw; i <= rw; i++) {
      ...
      for (let y = sy; y < sy + folH; y++) {
        ...
        if (d === 0 && rnd() < 0.10) col = leafGlow;              // lit blade
        if (d > 0 && rnd() < 0.05) col = PAL.moss1;               // moss patch
        if (d > 0 && rnd() < 0.028) col = PAL.cyan2;              // biolum fleck
        px(x, y, col);
      }
      ...
      for (let y = sy + folH; y < by; y++) {
        ...
        else if (f < 0.9) col = PAL.deepPurple;
        else col = stoneDeep;
        // lit left flank near the surface
        if (t < -0.55 && y - (sy + folH) < 4) col = stoneRim;
        else if (t < -0.3 && y - (sy + folH) < 2) col = PAL.stone2;
  ...
  // fireflies — faint biolum motes drifting between the islands
  for (let i = 0; i < 16; i++) {
